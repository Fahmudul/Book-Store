...
const ArrivalBooks = () => {
  ...
  useEffect(() => {
    fetch("https://raw.githubusercontent.com/Fahmudul/book-info/main/info.json")
      .then((res) => res.json())
      .then((data) => setArrivedBooks(data));
  }, []);

  return (
    <div className="mb-10 w-[87%] mx-auto lg:mt-10">
      <div className=" grid grid-cols-1 lg:grid-cols-4 place-items-center gap-y-4">
        {arrivedBooks.map((book) => (
          <div
            key={book.totalPages}
            className="w-[360px] h-[480px] p-6 books-card border-2 rounded-2xl hover:scale-105 duration-700"
          >
